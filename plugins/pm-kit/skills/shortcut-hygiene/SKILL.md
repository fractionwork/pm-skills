...
Audit a Shortcut project against `${CLAUDE_PLUGIN_ROOT}/skills/_shared/shortcut-conventions.md` and fix gaps.

Uses Shortcut MCP for reads where available. Uses `${CLAUDE_PLUGIN_ROOT}/skills/_shared/shortcut_ops.py` (REST API) for custom field creation and bulk operations. Auth: `SHORTCUT_API_TOKEN` env var.
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/_shared/shortcut_ops.py --setup
```
...
New work from external source enrichment (Step 6) goes to **Unscheduled** by default. Pre-stakeholder ideas (`add-card` skill triggers like "PM mentioned", "we might want to") land in **Inbox**. Stories move from Inbox → Unscheduled only after a stakeholder discussion comment is logged (the same promotion-gate rule as Asana — see `${CLAUDE_PLUGIN_ROOT}/skills/_shared/asana-conventions.md`). Stories move from Unscheduled → Ready for Development after refinement (description, estimate, priority, acceptance criteria).
