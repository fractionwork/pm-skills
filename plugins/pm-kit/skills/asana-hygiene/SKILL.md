...
Audit and fix an Asana project against `${CLAUDE_PLUGIN_ROOT}/skills/_shared/asana-conventions.md`.

Uses `${CLAUDE_PLUGIN_ROOT}/skills/_shared/asana_ops.py` for all operations. The script handles auth (auto-runs OAuth if needed) and provides reusable commands.

> **Tool precedence:** prefer the **first-party `asana` MCP** (`${CLAUDE_PLUGIN_ROOT}/skills/_shared/asana_mcp.py`); fall back to **`${CLAUDE_PLUGIN_ROOT}/skills/_shared/asana_ops.py`** for anything it lacks or when disconnected (it covers everything the MCP can't — create fields/sections/portfolios/tags, archive, upload files via `--attach-file`); do **not** use other Asana MCPs (official plugin / community servers are superseded). Full precedence + command surface in `${CLAUDE_PLUGIN_ROOT}/skills/_shared/asana-conventions.md` → "Tool precedence".
...
- **Stale INBOX:** flags items unmoved for >30 days (`INBOX_STALE_DAYS` in `${CLAUDE_PLUGIN_ROOT}/skills/_shared/asana_ops.py`)
...
In `${CLAUDE_PLUGIN_ROOT}/skills/_shared/asana_ops.py`, pass via the existing `params=` arg:
```python
