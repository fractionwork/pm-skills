...
The default is **BACKLOG**. Override to **INBOX** when the request signals an unvalidated idea — something that hasn't been discussed with stakeholders yet. See `${CLAUDE_PLUGIN_ROOT}/skills/_shared/asana-conventions.md` for the full flow.
...
For **Asana** (8 standard custom fields per `${CLAUDE_PLUGIN_ROOT}/skills/_shared/asana-conventions.md`):
...
- **Two admins per project** (`${CLAUDE_PLUGIN_ROOT}/skills/_shared/asana-conventions.md` → "Required admins"). If the project lacks two admins, the card you create is at risk of being orphaned. Run `/asana-hygiene` first if you're not sure.
